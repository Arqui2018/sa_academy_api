...
const url = process.env.STUDENTS_URL || 'students-ms';
...
const resolvers = {
  Query: {
    allStudents: (_) => getRequest(URL, ''),
    studentById: (_, { id }) => generalRequest(`${URL}/${id}`, 'GET'),
  },
  Mutation: {
    createStudent: (_, { student }) => generalRequest(`${URL}`, 'POST', student),
    updateStudent: (_, { id, student }) => generalRequest(`${URL}/${id}`, 'PUT', student),
    deleteStudent: (_, { id }) => generalRequest(`${URL}/${id}`, 'DELETE'),
  },
};

const url$1 = process.env.COURSES_URL || 'courses-ms';
...
const url$2 = process.env.GRADES_URL || 'grades-ms';
